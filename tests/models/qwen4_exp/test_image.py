...
fabricated fixture and the actual checkpoint.
...
# --------------------------------------------------------------------------------------
# The splice: _merge_multimodal. Unlike the Gemma4 template this module started from,
# upstream Qwen4ExpModel.forward already threads `batch` explicitly through every layer
# (no get_global_ctx() lookup inside the decoder stack) -- _merge_multimodal follows the
# same convention, taking `batch` as an explicit argument instead of reading a global.
# --------------------------------------------------------------------------------------
...
def test_merge_multimodal_writes_exactly_the_image_token_rows():
    ...
    batch = types.SimpleNamespace(mm_embeds=mm_embeds)

    out = Qwen4ExpModel._merge_multimodal(fake_self, input_ids, x, batch)
...
def test_merge_multimodal_asserts_on_slot_count_mismatch():
    ...
    with pytest.raises(AssertionError):
        Qwen4ExpModel._merge_multimodal(fake_self, input_ids, x, batch)
...
def test_merge_multimodal_no_op_when_batch_has_no_images():
    ...
    fake_self = types.SimpleNamespace(_image_token_id=999)
    batch = types.SimpleNamespace(mm_embeds=None)
...
def test_merge_multimodal_no_op_when_model_is_not_multimodal():
    ...
    batch = types.SimpleNamespace(mm_embeds=torch.randn(2, 6))
...
def test_merge_multimodal_no_op_missing_mm_embeds_attribute():
    """`getattr(batch, "mm_embeds", None)`, not `batch.mm_embeds`: a real Batch that has
    never carried multimodal state (no attribute at all, not just None) must not raise."""
    from freetoken.models.qwen4_exp.model import Qwen4ExpModel
    ...
    fake_self = types.SimpleNamespace(_image_token_id=999)
    batch = types.SimpleNamespace()  # no mm_embeds attribute at all
