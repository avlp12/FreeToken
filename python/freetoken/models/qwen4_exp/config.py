...
def _parse_vision_config(hf_config: Any, text_hidden_size: int) -> VisionConfig | None:
    ...
    # is_multimodal flows through parse_config into both model build (model.py) and weight
    # loading (weight.py's `_rename` gate), so returning None here keeps default `ft serve`
    # boot byte-for-byte unchanged.
