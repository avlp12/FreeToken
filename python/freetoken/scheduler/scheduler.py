...
class Scheduler(SchedulerIOMixin):
    ...
    def _fill_mm_embeds(self, msg: UserMsg) -> str | None:
        ...
        ``Qwen4ExpModel._merge_multimodal``). This is the one place in the online path
        where the request actually has a live model + device to hand the pixel tensor
        to, so preprocessing happens here rather than in the tokenizer (which only sizes
